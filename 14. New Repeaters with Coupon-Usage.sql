drop temporary table if exists tBasketsWithCouponsRaw;
create temporary table tBasketsWithCouponsRaw as
select t.Store
	  ,t.client_user_id
...
create index ixBaskets on tBasketsWithCouponsRaw (store, client_user_id, kpi_day);


delete from KPIsByDay where KPIName = 'New Repeaters with Coupon-Usage';
delete from KPIsByCalendarWeek where KPIName = 'New Repeaters with Coupon-Usage';
delete from KPIsByMonth where KPIName = 'New Repeaters with Coupon-Usage';
...
from tBasketsWithCouponsRaw;


update tBasketsCumulDay
set BasketsCumulative = (select sum(BasketCount) from tBasketsWithCouponsRaw t 
...
update tDayGlobal
set UsersWithRepeats = (select count(distinct t.client_user_id) from tBasketsCumulDay t where t.kpi_day = tDayGlobal.KPI_Day and t.BasketsCumulative = 2);

insert into KPIsByDay(KPIName, KPIDay, KPIValue)
select 'New Repeaters with Coupon-Usage' as KPIName 
	  ,KPI_DAY
...
set UsersWithRepeats = (select count(distinct t.client_user_id) from tBasketsCumulDay t where t.kpi_day = tDayGlobalStore.KPI_Day and t.store = tDayGlobalStore.store and t.BasketsCumulative = 2);
...
set UsersWithRepeats = (select count(distinct t.client_user_id) from tBasketsCumulDay t 
						where case
							  ...
							  end = tCWGlobal.KPI_Day and t.BasketsCumulative = 2);
...
set UsersWithRepeats = (select count(distinct t.client_user_id) from tBasketsCumulDay t 
						where case
							  ...
							  end = tCWGlobalStore.KPI_Day and t.store = tCWGlobalStore.store and t.BasketsCumulative = 2);
...
update tMonthGlobal
set UsersWithRepeats = (select count(distinct t.client_user_id) from tBasketsCumulDay t 
                        where date_format(t.kpi_day, '%Y.%m') = tMonthGlobal.KPI_Day and t.BasketsCumulative = 2);
...
set UsersWithRepeats = (select count(distinct t.client_user_id) from tBasketsCumulDay t 
                        where date_format(t.kpi_day, '%Y.%m') = tMonthGlobalStore.KPI_Day and t.store = tMonthGlobalStore.store and t.BasketsCumulative = 2);
